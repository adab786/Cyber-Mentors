      ...
      <div className="p-4 bg-zinc-900 min-h-screen  bg-[url('https://assets.lummi.ai/assets/QmfEF5VuQ7Tm8JSXm3sp6HHuEm24JWijG6q3sKiVKQQsxf?auto=format&w=1500')]">
        ...
        <div className="flex flex-wrap justify-center gap-4  ">
          {data.map((item) => (
            <div
              key={item.id}
              className=" p-4 rounded-lg w-64 bg-transparent border-blue-600 border-2 hover:-translate-y-4 transform transition-all duration-300"
            >
              <img
                src={item.img}
                alt={item.title}
              ...
              <div className="flex  justify-between items-center">
                ...
                <button
                  onClick={() =>
                    handleDownload(item.img, `zine-${item.id}.jpg`)
                  }
                  className="hover:scale-105 hover:bg-lime-800  py-1 px-4    bg-blue-500 rounded-lg text-white"
                >
                  view
                </button>
              </div>
